...
**更新时间：** 2026-07-20（Immich Provider V0.1 工程化接入）
...
正式入口共享一个 database engine、PostgreSQLRepository 和 Matcher，并为
Nextcloud、Immich 分别运行独立的 `SyncEngine.sync_once()`。当前是顺序同步，
不是多 Provider 并行同步。
...
Identity V1 已经在 Nextcloud 和 Immich 两个真实 Provider 上复用，验证了当前
ProviderFact、Matcher、SyncEngine、Repository 和 World Model 边界可以承载
第二个 Provider。

下一阶段不应继续无边界扩展底层抽象。优先顺序建议为：

1. 在受控环境持续运行 Nextcloud 与 Immich integration tests；
2. 通过真实 metadata-only 变更继续评估 Immich `updatedAt` 的同步成本；
