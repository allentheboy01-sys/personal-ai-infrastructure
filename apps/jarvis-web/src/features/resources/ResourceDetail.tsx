...
import { Eye, FileText, Image, ImageOff, Info, Mail, Maximize2, ShieldCheck, X } from 'lucide-react'
...
export function ResourceDetail({ resource }: { resource: ResourceView }) {
  ...
  return (
    <div className="detail-content resource-detail">
      ...
      </Dialog.Root> : resource.presentation.kind === 'image' && previewFailed ? <div className="detail-hero image"><ImageOff size={28} /><span>Preview unavailable</span></div> : <div className={`detail-hero ${resource.presentation.kind}`}><Icon size={28} /><span>{resource.presentation.label}</span></div>}
