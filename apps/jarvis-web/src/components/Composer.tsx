...
export function Composer({ centered = false, running = false, onSubmit }: { centered?: boolean; running?: boolean; onSubmit?: (value: string) => void }) {
    ...
    <div className={`composer-wrap ${centered ? 'centered' : ''}`}>
      <div className={`composer ${running ? 'is-running' : ''}`}>
        ...
        <div className="composer-actions">
          ...
          {running ? <button className="send-button stop" aria-label="Stop response"><Square size={13} fill="currentColor" /></button> : <button className="send-button" aria-label="Send message" onClick={submit} disabled={!value.trim()}><ArrowUp size={18} /></button>}
        </div>
      </div>
    </div>
  )
